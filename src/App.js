...
import { domainContractAddress, ownerAddress, tld } from './config';
...
const App = () => {
  ...
  const switchNetwork = async () => {
    if (window.ethereum) {
      try {
        // Try to switch to the Mainnet
        await window.ethereum.request({
          method: 'wallet_switchEthereumChain',
          params: [{ chainId: '0x89' }], // 0x89 is hexadecimal for Mainnet
        });
      } catch (error) {
  ...
  const fetchMints = async () => {
    try {
      ...
      if (ethereum) {
        ...
        const check = (await ownerAddress.toLowerCase()) === currentAccount;
  ...
  const withdrawBalance = async () => {
    ...
    try {
      ...
      if (ethereum) {
        ...
        await tx.wait();
        console.log(
          'Withdrawing success https://polygonscan.com/tx/' + tx.hash
        );
      }
    } catch (err) {
      console.log(err);
          ...
          <div className="button-container">
            {loading === true ? (
              ...
              <>
                ...
                {/*If the current user is the contract owner, the withdraw button will be returned instead */}
                {owner ? (
                  <button
                    className="cta-button mint-button"
                    disabled={loading}
              ...
              return (
                <div className="mint-item" key={index}>
                  <div className="mint-row">
                    <a
                      className="link"
                      href={`https://opensea.io/assets/matic/${domainContractAddress}/${mint.id}`}
                      target="_blank"
          ...
          <header>
            <div className="left">
              ...
              <p className="subtitle">
                Blockchain's custom name, runs on Polygon L2
              </p>
            </div>
